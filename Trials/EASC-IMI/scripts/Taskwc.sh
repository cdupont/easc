...
function changewm(){
        actualwm=$1;
        nextwm=$2;
        nvmls=$(echo $nextwm | grep -o "WC[0-9]\+" | grep -o "[0-9]\+");
        type=$3;
        tasca="WC";
...
eselzero=$(echo $nextwm | grep -ic "WC0")
data=$(date +%Y-%m-%d)
...
nvmlsabans=$(echo $actualwm | grep -o "WC[0-9]\+" | grep -o "[0-9]\+");
...
pirexisteix=$(sqlite3 -init resource/init.sql  resource/dc4cities.sqlt "SELECT COUNT(*) FROM virtual_machines WHERE ip='10.0.200.61';")
...
ruby scripts/virtualnop.rb 4 release 10.0.200.61
...
while [ -z $piretrixip ]; do
  piretrixip=$(econe-describe-instances -K user -S password -U http://sunstone:4568/ | grep $identificador | grep -o "10.0.200.[0-9]\+")
done
...
piretrixip="10.0.200.61"
	...
	do
		...
		sleep 5;
		ippadicaptura=$(econe-describe-instances -K user -S password -U http://sunstone:4568/ | grep "$identificador" | grep -o "10.0.200.[0-9]\+");
...
consulta=$(sqlite3 -init resource/init.sql  resource/dc4cities.sqlt "SELECT wm_to_vm.vmid, ip FROM wm_to_vm, virtual_machines WHERE virtual_machines.vmid=wm_to_vm.vmid AND activity='WebCrawling' AND date='"$data"' AND wm='"$nextwm"' AND ip!='10.0.200.61';")
...
quanteshanquedat=$(sqlite3 -init resource/init.sql  resource/dc4cities.sqlt "SELECT COUNT(*) FROM virtual_machines, wm_to_vm WHERE activity='WebCrawling' AND date='"$data"' AND wm='"$nextwm"' AND virtual_machines.vmid=wm_to_vm.vmid AND ip!='10.0.200.61';")
if [ "$quanteshanquedat" -gt "$nvmls" ]; then

   araquantesmates=$(echo "$quanteshanquedat-$nvmls"|bc -l)
   consulta=$(sqlite3 -init resource/init.sql  resource/dc4cities.sqlt "SELECT wm_to_vm.vmid FROM virtual_machines, wm_to_vm WHERE activity='WebCrawling' AND date='"$data"' AND wm='"$nextwm"' AND virtual_machines.vmid=wm_to_vm.vmid AND ip!='10.0.200.61';")
   arrayquery=($( for j in $consulta ; do echo $j ; done ))
...
if [ "$eselzero" -eq "1" ];then
    rvm use system 1>/dev/null
    pirid=$(sqlite3 -init resource/init.sql  resource/dc4cities.sqlt "SELECT vmid FROM virtual_machines WHERE ip='10.0.200.61';")
...
source /home/dc4cities/.rvm/scripts/rvm
rvm use ruby-2.1.4 1>/dev/null
ruby scripts/virtualnop.rb 4 hold 10.0.200.61
fi
...
imatgepiretrix="ami-00000063"
imatgepadicatcaptura="ami-00000062"
