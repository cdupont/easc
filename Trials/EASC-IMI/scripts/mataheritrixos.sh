...
consulta=$(sqlite3 -init resource/init.sql  resource/dc4cities.sqlt "SELECT ip FROM wm_to_vm, virtual_machines WHERE activity='WebCrawling' AND date='"$data"' AND wm='"$actualwm"' AND wm_to_vm.vmid=virtual_machines.vmid AND ip!='10.0.200.61';")
...
do
        ...
        then
          ...
          if [ -e "resource/statusmata.html" ]; then
			...
			timeout -s 9 1m scripts/encendreheritrixos.sh $ipmaquina 10.0.200.61 &
			let i=i+1
                  fi
          fi
        fi
