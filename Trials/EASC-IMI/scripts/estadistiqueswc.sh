...
eszerocanviant=$(echo "$actualwmcanviant"|grep -c "WC0")
...
eszerocanviat=$(echo "$actualwmcanviat"|grep -c "WC0")
...
if [ "$existeixwmdiaactual" -eq "0" ]; then
        wmahir=$(sqlite3 -init resource/init.sql  resource/dc4cities.sqlt "SELECT wm FROM working_modes WHERE date='"$ahir"' AND activity='WebCrawling' AND status='canviat' AND wm!='WC0';")
        if [ -n "$wmahir" ]; then
...
consulta=$(sqlite3 -init resource/init.sql  resource/dc4cities.sqlt "SELECT ip FROM wm_to_vm, virtual_machines WHERE virtual_machines.vmid=wm_to_vm.vmid AND activity='WebCrawling' AND date='"$avui"' AND wm='"$actualwm"' AND ip!='10.0.200.61';")
...
#canvies a WC0 poso a 0 tot excepte el que ja s'ha fet
existeix=$(sqlite3 -init resource/init.sql  resource/dc4cities.sqlt "SELECT COUNT(*) FROM progress WHERE activity='WebCrawling' AND date='"$avui"'");
