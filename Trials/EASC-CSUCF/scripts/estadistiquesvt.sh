...
function calcular(){
  ...
  do
      ...
      if [ "$quants" -eq "0" ]; then
	sqlite3 -init resource/init.sql  resource/dc4cities.sqlt "INSERT INTO fets VALUES ('"$activity"','"$nomvideo"')";
...
avui=$(date +"%Y-%m-%d")
surt=0
actualwmcanviant=$(sqlite3 -init resource/init.sql  resource/dc4cities.sqlt "SELECT wm FROM working_modes WHERE activity='"$activity"' AND date='"$avui"' AND status='canviant';");
eszerocanviant=$(echo "$actualwmcanviant"|grep -c "${vtname}0")

actualwmcanviat=$(sqlite3 -init resource/init.sql  resource/dc4cities.sqlt "SELECT wm FROM working_modes WHERE activity='"$activity"' AND date='"$avui"' AND status='canviat';");
eszerocanviat=$(echo "$actualwmcanviat"|grep -c "${vtname}0")
...
existeixwmdiaactual=$(sqlite3 -init resource/init.sql  resource/dc4cities.sqlt "SELECT COUNT(*) FROM working_modes WHERE date='"$avui"' AND activity='"$activity"' AND status='canviat';")
if [ "$existeixwmdiaactual" -eq "0" ]; then
	wmahir=$(sqlite3 -init resource/init.sql  resource/dc4cities.sqlt "SELECT wm FROM working_modes WHERE date='"$ahir"' AND activity='"$activity"' AND status='canviat' AND wm!='"${vtname}0"';")
	if [ -n "$wmahir" ]; then
		sqlite3 -init resource/init.sql  resource/dc4cities.sqlt "INSERT INTO working_modes VALUES ('"$avui"', '"$activity"', '"$wmahir"', 'canviat');";
		sqlite3 -init resource/init.sql  resource/dc4cities.sqlt "UPDATE wm_to_vm SET date='"$avui"' WHERE date='"$ahir"' AND activity='"$activity"' AND wm='"$wmahir"';";
	fi
fi
...
consulta=$(sqlite3 -init resource/init.sql  resource/dc4cities.sqlt "SELECT ip FROM wm_to_vm, virtual_machines WHERE virtual_machines.vmid=wm_to_vm.vmid AND activity='"$activity"' AND date='"$avui"' AND wm='"$actualwm"';")
...
existeix=$(sqlite3 -init resource/init.sql  resource/dc4cities.sqlt "SELECT COUNT(*) FROM progress WHERE activity='"$activity"' AND date='"$avui"'");
if [ "$existeix" -eq "0" ]; then
 sqlite3 -init resource/init.sql  resource/dc4cities.sqlt "INSERT INTO progress VALUES ('"$activity"', '"$avui"', $mbdonetotal, $ultimmintotal, $ultimmintotal, '"$segonsara"');"
else
	sqlite3 -init resource/init.sql  resource/dc4cities.sqlt "UPDATE progress SET quantity=quantity+$mbdonetotal, last_instant_progress=last_instant_progress+$ultimmintotal, last_minute=$ultimmintotal, date_last_minute='"$segonsara"' WHERE activity='"$activity"' AND date='"$avui"';"
#sqlite3 -init resource/init.sql  resource/dc4cities.sqlt "UPDATE progress SET quantity=quantity+$mbdonetotal, last_instant_progress=$progressinmediat+quantity, last_minute=$ultimmintotal, date_last_minute='"$segonsara"' WHERE activity='"$activity"' AND date='"$avui"'"
fi
...
if [ "$existeix" -gt "0" ]; then
	sqlite3 -init resource/init.sql  resource/dc4cities.sqlt "UPDATE progress SET last_minute=0, date_last_minute='"$segonsara"' WHERE activity='"$activity"' AND date='"$avui"'"
#sqlite3 -init resource/init.sql  resource/dc4cities.sqlt "UPDATE progress SET last_instant_progress=quantity, last_minute=0, date_last_minute='"$segonsara"' WHERE activity='"$activity"' AND date='"$avui"'"
fi
