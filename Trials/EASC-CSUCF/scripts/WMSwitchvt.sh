...
function check_vm_host(){
	...
	consulta=$(sqlite3 -init resource/init.sql  resource/dc4cities.sqlt "SELECT vmid FROM wm_to_vm WHERE activity='"$activityvt"';");
...
#ens assegurem que no vagi massa rapid
for kk in `seq 1 2`
do
...
function check_wm_yesterday(){
	...
	existeixwmdiaactual=$(sqlite3 -init resource/init.sql  resource/dc4cities.sqlt "SELECT COUNT(*) FROM working_modes WHERE date='"$avui"' AND activity='"$activityvt"' AND status='canviat';")
	if [ "$existeixwmdiaactual" -eq "0" ]; then
		wmahir=$(sqlite3 -init resource/init.sql  resource/dc4cities.sqlt "SELECT wm FROM working_modes WHERE date='"$ahir"' AND activity='"$activityvt"' AND status='canviat' AND wm!='"${vtname}0"';")
		if [ -n "$wmahir" ]; then
			sqlite3 -init resource/init.sql  resource/dc4cities.sqlt "INSERT INTO working_modes VALUES ('"$avui"', '"$activityvt"', '"$wmahir"', 'canviat');";
			sqlite3 -init resource/init.sql  resource/dc4cities.sqlt "UPDATE wm_to_vm SET date='"$avui"' WHERE date='"$ahir"' AND activity='"$activityvt"' AND wm='"$wmahir"';";
		fi
	fi
}
...
actualwm=$(sqlite3 -init resource/init.sql  resource/dc4cities.sqlt "SELECT wm FROM working_modes WHERE activity='"$activityvt"' AND date='"$data"' AND status='canviat';");

if [ -z "$actualwm" ]; then
	sqlite3 -init resource/init.sql  resource/dc4cities.sqlt "INSERT INTO working_modes VALUES ('"$data"', '"$activityvt"', '"${vtname}0"', 'canviat');";
	actualwm="${vtname}0"
fi

if [ "$actualwm" != "$nextwm" ]; then

sqlite3 -init resource/init.sql  resource/dc4cities.sqlt "INSERT INTO working_modes VALUES ('"$data"', '"$activityvt"', '"$nextwm"', 'canviant');";

eszero=$(echo $nextwm|grep -c "${vtname}0")
if [ "$eszero" -eq "0" ]; then
        if ! [ "$(pidof -x estadistiquesvt.sh)" ]; then
           nohup scripts/estadistiquesvt.sh $datacenter 2>/dev/null&
        fi
fi
...
check_nodes_all_on
  ...
  scripts/Taskvt.sh changewm $actualwm $nextwm $datacenter

sqlite3 -init resource/init.sql  resource/dc4cities.sqlt "DELETE FROM working_modes WHERE activity='"$activityvt"' AND date='"$data"' AND wm='"$actualwm"' AND status='canviat';"
sqlite3 -init resource/init.sql  resource/dc4cities.sqlt "UPDATE working_modes SET status='canviat' WHERE  activity='"$activityvt"' AND date='"$data"' AND wm='"$nextwm"' AND status='canviant';";
