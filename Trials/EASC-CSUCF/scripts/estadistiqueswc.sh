...
function calcular(){
  ...
  do
      ...
      if [ "$quants" -eq "0" ]; then
	sqlite3 -init resource/init.sql  resource/dc4cities.sqlt "INSERT INTO fets VALUES ('"$activity"','"$nomweb"')";
...
if [ "$existeixwmdiaactual" -eq "0" ]; then
        wmahir=$(sqlite3 -init resource/init.sql  resource/dc4cities.sqlt "SELECT wm FROM working_modes WHERE date='"$ahir"' AND activity='"$activity"' AND status='canviat' AND wm!='"${wcname}0"';")
        if [ -n "$wmahir" ]; then
...
consulta=$(sqlite3 -init resource/init.sql  resource/dc4cities.sqlt "SELECT ip FROM wm_to_vm, virtual_machines WHERE virtual_machines.vmid=wm_to_vm.vmid AND activity='"$activity"' AND date='"$avui"' AND wm='"$actualwm"' AND ip!='"$piretrixip"';")
...
if [ "$existeix" -eq "0" ]; then
 sqlite3 -init resource/init.sql  resource/dc4cities.sqlt "INSERT INTO progress VALUES ('"$activity"', '"$avui"', $websdonetotal, $ultimmintotal, $ultimmintotal, '"$segonsara"')"
else
	sqlite3 -init resource/init.sql  resource/dc4cities.sqlt "UPDATE progress SET quantity=quantity+$websdonetotal, last_instant_progress=last_instant_progress+$ultimmintotal, last_minute=$ultimmintotal, date_last_minute='"$segonsara"' WHERE activity='"$activity"' AND date='"$avui"'"
# sqlite3 -init resource/init.sql  resource/dc4cities.sqlt "UPDATE progress SET quantity=quantity+$websdonetotal, last_instant_progress=$progressinmediat+quantity, last_minute=$ultimmintotal, date_last_minute='"$segonsara"' WHERE activity='"$activity"' AND date='"$avui"'"
fi
