...
public class MonitorZabbix extends Thread implements MonitorInterface {
    ...
    private String dcName;
        public MonitorZabbix(String dcName){
    ...
    private String executeRpcMethod(JSONObject jsonObj) {
        ...
        try {
            ...
            HttpPost request = new HttpPost(ConstantsZabbix.zabbixips.get(this.dcName));
    ...
    private void authenticateUser() {
        try {
            ...
            jsonObj.put("params", (new JSONObject().put("user", ConstantsZabbix.zabbixuser.get(this.dcName)).put("password", ConstantsZabbix.zabbixpassword.get(this.dcName))));
            jsonObj.put("id", 0);
    ...
    public int getValuefromKey(String hostname, String keyzabbix, long from, long till, boolean mitjana){
        ...
        int itemvalue = 0;
        try{
        String key = ConstantsZabbix.zabbixkeys.get(this.dcName+"."+keyzabbix);
        JSONObject getHostReqObj = new JSONObject();
        getHostReqObj.put("jsonrpc", "2.0");
...
public int getPowerNodes(long from, long till, List<String> hostsEnable){
	LocalDateTime now = LocalDateTime.now();
	if (java.time.Duration.between(last_reading, now).getSeconds()>120){
		serverPower=new HashMap<String,Integer>();
		last_reading=LocalDateTime.now();
	}
        int power=0;
        Iterator<String> it=ConstantsZabbix.machinesblade.iterator();
        while (it.hasNext()){
                power+=getServerPower(it.next(), from, till);
        }
        it=hostsEnable.iterator();
        while (it.hasNext()){
                power+=getServerPower(it.next(), from, till);
        }
        return power;
