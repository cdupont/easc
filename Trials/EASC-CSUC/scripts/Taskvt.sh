...
function changewm(){
        actualwm=$1;
        nextwm=$2;
        nvmls=$(echo $nextwm | grep -o "VTWM[0-9]\+" | grep -o "[0-9]\+");
        type=$3;
        tasca="VTWM";
...
nvmlsabans=$(echo $actualwm | grep -o "VTWM[0-9]\+" | grep -o "[0-9]\+");
...
nvmls=$(echo "$nvmls-$nvmlsabans"|bc -l)
	...
	do
		...
		sleep 5;
		iptranscoding=$(econe-describe-instances -K user -S password -U http://sunstone:4568/ | grep "$identificador" | grep -o "84.88.31.[0-9]\+");
