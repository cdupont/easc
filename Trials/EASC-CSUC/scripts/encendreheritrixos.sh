...
###
        ...
        ssh -i /home/ois/.ssh/user/id_rsa -t -t -o "StrictHostKeyChecking no" -n user@$IPPIRETRIX -p2122 "sudo /dades/eines/SE2/loader/dc4cities.sh $i $webaindexar" >> /tmp/heritrix.log&
        sleep 10
...
while [ "$piretrixok" -eq "1" ]
do
  piretrixok=$(nc -z -w5 $IPPIRETRIX 2122; echo $?)
sleep 2
done
