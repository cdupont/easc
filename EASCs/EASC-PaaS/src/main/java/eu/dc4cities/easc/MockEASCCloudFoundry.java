...
public class MockEASCCloudFoundry extends EASC {
	
	public MockEASCCloudFoundry(String configDirectory, PaaSMonitor monitorFactory) {
		super(configDirectory,  monitorFactory);
		...
		MockCFController cfCli = new MockCFController(cfConfig.getApiEndpoint(), cfConfig.getUsername(), cfConfig.getPassword());
		cfCli.login();
		monitorFactory.setPaaSController(cfCli);
		monitorFactory.init();
		
		//Defining a CloudFoundry WMM, instead of DefaultWMM 
		WorkingModeManager wmm = new MockCFWorkingModeManager(this.getAppConfig(), cfCli);
